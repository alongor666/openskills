...
> Tailscale、不暴露公网，全程公司内部网络；同事可脱离 AI 独立照做。默认 Windows ＋ 本地管理员；
> 无管理员 / 非 Windows 先看「第 0 步」的降级与适配路径。
>
...
### 第 0 步：先探后问（probe-env.ps1）

动手前先跑只读体检——能探到的不要问人，探不到的才向用户确认：

```powershell
pwsh -File scripts\probe-env.ps1
```

| 探什么 | 怎么探 | 探明后怎么适配 |
|---|---|---|
| 管理员权限 | `net session` 被拒 = 非管理员 | 走本节末尾「无管理员降级路径」 |
| 数据库类型 | 服务清单 / 1433·3306·5432 监听 / 数据文件扩展名 | 决定通道与第 2 步导出命令 |
| 网络 profile | `Get-NetConnectionProfile` 显示 Public | Public 拦 SMB；改档影响整机策略，先问用户 / IT |
| 数据盘容量 | 各盘剩余空间 | DbStage / DbShare 选容量最大的数据盘 |
| 共享名冲突 | `Get-SmbShare` | DbShare 重名就换名 |
| 导出工具 | sqlite3 / mysqldump / pg_dump / sqlcmd 是否在 PATH | 缺则换导出法或先补装 |

再向用户确认四件事（AI Agent 用各自原生提问能力；自助操作者逐项自答；未确认按保守默认）：
...
| 通道 A / B（要不要实时） | A |
| 快照周期（每小时 / 每天 / 每周） | 每天 |
| 敏感字段与审批状态 | 视为敏感 → 绝对脱敏 ＋ 审批通过后才开放 |

**无管理员降级路径**：① 请 IT 在既有合规共享上开只读子目录；② 数据库端口本已放行 → 直接走通道 B 只读账号；③ 提 IT 工单开通。禁止绕权限硬来。
...
- **绝对脱敏（法律红线，不是可选项）**：姓名 / 证件号 / 手机号等个人敏感信息，能不导出的一律不导出；确需保留关联性的必须加盐哈希（SHA-256 等）或掩码（如 138****5678、610***********1234），默认不落明文。未脱敏 = 不开放。遵守《个人信息保护法》与公司数据安全制度，敏感范围先走审批再开放。
